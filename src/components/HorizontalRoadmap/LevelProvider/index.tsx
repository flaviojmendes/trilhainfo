...
export const LevelProvider = ({ children }: LevelProviderProps) => {
  const [items, setItems] = useLocalStorage<Map<string, boolean>>('items', new Map(), {
...
export const useLevelRoadmapActions = (item: RoadmapItem) => {
  const context = useContext(LevelContext);
  if (!context) {
    throw new Error('`useLevelRoadmapActions` must be used within `LevelProvider`');
  }

  return {
    isAllContentRead: () => context.isAllContentRead(item.label, item.children?.length ?? -1),
...
export const useSectionRoadmapActions = (section: RoadmapItem) => {
  const context = useContext(LevelContext);
  if (!context) {
    throw new Error('`useSectionRoadmapActions` must be used within `LevelProvider`');
  }
